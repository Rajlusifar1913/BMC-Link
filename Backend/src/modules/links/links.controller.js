...
export const getCreatorLinks = asyncHandler(async (req, res) => {
    ...
    const links = await linkService.getCreatorLinks(
        req.user.id,
        req.query
    );
...
export const toggleLink = asyncHandler(async (req, res) => {
    ...
    const link = await linkService.toggleLink(
        req.user.id,
        req.params.id
    );
...
export const duplicateLink = asyncHandler(async (req, res) => {
    ...
    const link = await linkService.duplicateLink(
        req.user.id,
        req.params.id
    );
...
export const getPublicLinks = asyncHandler(async (req, res) => {
    ...
    const links = await linkService.getPublicLinks(
        req.params.username
    );
    ...
    return res.status(200).json(
        new ApiResponse(
            200,
            links,
            "Public links fetched successfully"
        )
    );
});